...
class StorageService:
    ...
    def sanitize_filename(self, filename: str) -> str:
        ...
        # Convert some special Vietnamese characters that don't decompose
        name = name.replace("Đ", "D").replace("đ", "d")
        # Normalize and remove diacritics -> ASCII
        nfkd = unicodedata.normalize("NFKD", name)
        name_ascii = nfkd.encode("ascii", "ignore").decode("ascii")
        # replace spaces with underscores
        name_ascii = name_ascii.replace(" ", "_")
        # Allow only a safe subset of characters
        name_ascii = "".join(c for c in name_ascii if c.isalnum() or c in "_.-()")
        if not name_ascii:
            name_ascii = "file"
        timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        # Ensure extension is ASCII-safe too
        ext_nfkd = unicodedata.normalize("NFKD", ext)
        ext_ascii = ext_nfkd.encode("ascii", "ignore").decode("ascii")
        return f"{name_ascii}_{timestamp}{ext_ascii}"

    def _ascii_safe(self, obj) -> str:
        """Return an ASCII-only, display-safe string for exceptions/messages.

        Non-ASCII characters are replaced with backslash escapes so printing
        on cp1252 consoles won't raise UnicodeEncodeError.
        """
        try:
            s = str(obj)
            return s.encode("utf-8", "backslashreplace").decode("ascii")
        except Exception:
            return repr(obj)
    ...
    def upload_file(
        ...
        try:
            ...
            try:
                resp = (
                    self.supabase.table("files_metadata")
                    .upsert({"filename": safe_filename, "metadata": metadata})
                    .execute()
                )
                ...
                try:
                    ...
                    print(
                        f"CRITICAL: failed to remove orphaned uploaded file {safe_filename}: {self._ascii_safe(remove_err)}"
                    )
                print(
                    f"Error: failed to upsert files_metadata for {safe_filename}: {self._ascii_safe(e)}"
                )
                return {
                    "success": False,
                    "error": "Failed to persist file metadata (encryption key). Upload aborted.",
                }

            return {"success": True, "filename": safe_filename}
        except Exception as e:
            # Use ASCII-safe message to avoid Windows cp1252 encode errors when printing
            print(f"Error uploading file: {self._ascii_safe(e)}")
            return {"success": False, "error": self._ascii_safe(e)}
    ...
    def download_file(self, filename: str):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Warning: could not read files_metadata for {filename}: {self._ascii_safe(e)}"
                )
                metadata = {}
            ...
            try:
                ...
                print(f"Error downloading file {filename}: {self._ascii_safe(e)}")
                return None
            ...
            if (
                metadata
                and metadata.get("encrypted") == "true"
                and not metadata.get("encryption_key")
            ):
                print(
                    f"Detected encrypted file {filename} but encryption_key missing in metadata"
                )
            ...
            if (not metadata) and isinstance(data, (bytes, bytearray)):
                try:
                    ...
                    if data.startswith(b"gAAAA"):
                        print(
                            f"Detected encrypted blob for {filename} but missing metadata/key"
                        )
                        return {"error": "encrypted_missing_key", "filename": filename}
                except Exception:
                    pass
            ...
            if (
                metadata
                and metadata.get("encrypted") == "true"
                and metadata.get("encryption_key")
            ):
                try:
                    decrypted = self.decrypt_file(
                        data, metadata["encryption_key"].encode()
                    )
                    ...
                    if original_filename.endswith(".doc"):
                        if not decrypted.startswith(
                            ...
                            raise ValueError(
                                "Decrypted file does not match .doc format"
                            )
                        ...
                        if not decrypted.startswith(b"PK"):
                            raise ValueError(
                                "Decrypted file does not match .docx format"
                            )
                    return (
                        decrypted,
                        metadata.get("original_filename", filename),
                        metadata.get("original_type", "application/octet-stream"),
                    )
                except Exception as e:
                    print(f"Error decrypting file {filename}: {self._ascii_safe(e)}")
                    return None
            ...
            print(f"Error when downloading file {filename}: {self._ascii_safe(e)}")
            return None
    ...
    def delete_file(self, filename: str) -> bool:
        try:
            ...
            try:
                self.supabase.table("files_metadata").delete().eq(
                    "filename", filename
                ).execute()
            except Exception:
                pass
            ...
            print(f"Error deleting file: {self._ascii_safe(e)}")
            return False
    ...
    def list_files(self, current_user=None, public_only=False):
        ...
        try:
            ...
            for f in files:
                ...
                size_from_storage = (
                    storage_meta.get("size")
                    or storage_meta.get("contentLength")
                    or f.get("size")
                )
                created_at = (
                    f.get("created_at")
                    or f.get("updated_at")
                    or storage_meta.get("lastModified")
                )
                ...
                merged = {
                    ...
                    "original_size": custom_meta.get(
                        "original_size", size_from_storage
                    ),
                    "original_type": custom_meta.get(
                        "original_type",
                        storage_meta.get("mimetype", "application/octet-stream"),
                    ),
                ...
                try:
                    ...
                    if isinstance(ts, str) and "T" in ts:
                        ...
                        merged["time_display"] = vietnam_time.strftime(
                            "%d/%m/%Y %H:%M:%S"
                        )
                    else:
                ...
                results.append({"name": name, "id": f.get("id"), "metadata": merged})

            # Sort by upload time (newest first)
            results.sort(
                key=lambda x: x["metadata"].get("uploaded_at", ""), reverse=True
            )
            return results
        except Exception as e:
            # Distinguish network/connection errors from empty list: return None on error
            print(f"Error listing files: {self._ascii_safe(e)}")
            return None

    def get_public_url(self, filename: str):
        """Return a public URL for the object if available (may require public bucket)."""
        try:
            resp = self.supabase.storage.from_(self.bucket_name).get_public_url(
                filename
            )
            # SDKs vary: could be dict or string
            if isinstance(resp, dict):
                return (
                    resp.get("publicURL") or resp.get("public_url") or resp.get("url")
                )
            return resp
        except Exception as e:
            print(
                f"Warning: get_public_url failed for {filename}: {self._ascii_safe(e)}"
            )
            return None
    ...
    def create_signed_url(self, filename: str, expires=60):
        ...
        try:
            resp = self.supabase.storage.from_(self.bucket_name).create_signed_url(
                filename, expires
            )
            if isinstance(resp, dict):
                return (
                    resp.get("signedURL") or resp.get("signed_url") or resp.get("url")
                )
            return resp
        except Exception as e:
            print(
                f"Warning: create_signed_url failed for {filename}: {self._ascii_safe(e)}"
            )
            return None
